...
namespace SampleProject.Domain.Applications.Behavior
{
    ...
    public class RedLockPipelineBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : notnull
    {
        ...
        private TimeSpan _expiryTime = TimeSpan.FromSeconds(30.0);
        private TimeSpan _waitTime = TimeSpan.FromSeconds(20.0);
        private TimeSpan _retryTime = TimeSpan.FromMilliseconds(100.0);
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            if (request is IRequireRedLock lockableRequest)
            {
                ...
                throw new Exception($"Failed to acquire RedLock. Key: {lockableRequest.LockKey}, Command: {request}.");
            }
            else
            {
                return await next();
